...
class CricketTargetScorePredictor:
    ...
    def fit(self, df, epochs=10, batch_size=32, validation_split=0.2, 
            model_file_path='../models/cricket_target_score_model.pth', 
            preprocess_file_path='../models/cricket_preprocess.joblib'):
    ...
    def save_model(self, model_file_path='../models/cricket_target_score_model.pth', 
                   preprocess_file_path='../models/cricket_preprocess.joblib'):
    ...
    def load_model(self, model_file_path='../models/cricket_target_score_model.pth', 
                   preprocess_file_path='../models/cricket_preprocess.joblib'):
...
def run_sweep(df, hyperparams):
    ...
    best_val_loss = float('inf')
    best_run_id = None
    best_model_path = 'models/cricket_target_score_model_best.pth'
    best_preprocess_path = 'models/cricket_preprocess_best.joblib'
    ...
    for hidden_dims, dropout_rate, learning_rate in param_combinations:
        with mlflow.start_run():
            ...
            model_file_path = f'../models/cricket_target_score_model_{mlflow.active_run().info.run_id}.pth'
            preprocess_file_path = f'../models/cricket_preprocess_{mlflow.active_run().info.run_id}.joblib'
...
if __name__ == "__main__":
    ...
    sample_data = {
        'match_id': range(1000),
        'inning': np.random.randint(1, 3, 1000),
        'total_runs': np.random.randint(0, 400, 1000),
        'is_wicket': np.random.randint(0, 2, 1000),
        'over': np.random.randint(1, 51, 1000),
        'team1': np.random.choice(['TeamA', 'TeamB', 'TeamC'], 1000),
        'team2': np.random.choice(['TeamA', 'TeamB', 'TeamC'], 1000),
        'toss_winner': np.random.choice(['TeamA', 'TeamB', 'TeamC'], 1000),
        'venue': np.random.choice(['Venue1', 'Venue2', 'Venue3'], 1000),
        'batting_strength': np.random.uniform(0, 100, 1000),
        'bowling_strength': np.random.uniform(0, 100, 1000),
        'target_score': np.random.randint(100, 400, 1000)
    }
